...
It is important to note that `tidymass_shiny` utilizes the `shinyFiles` package, which is inherently a local file management system. Theoretically, `shinyFiles` cannot directly access local files. This means you must first create a working directory under your user account on the server and upload the raw data to be analyzed (e.g., via SFTP) to this server directory before proceeding.
